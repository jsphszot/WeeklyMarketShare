...
DECLARE Week0 INT64 DEFAULT EXTRACT(ISOWEEK FROM CURRENT_DATE())-{back_weeks}; -- backweeks to 1 when running in BQ
...
WITH cookie2 AS (
    SELECT 
    CASE 
        WHEN Destino = 'CHILE' THEN CONCAT(Origen, ' - ', Destino) 
        WHEN Destino IN ("VCP", "GRU") THEN CONCAT(Origen, ' - ', "SAO")
        WHEN Origen = 'USA' AND Destino IN ('LIM') THEN CONCAT(Origen, ' - ', Destino)
        ELSE CONCAT(Origen, ' - ', Destino)
    END AS Grupo, -- TipoVuelo has been excluded
    Owner,
